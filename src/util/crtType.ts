...
const monitorLineSpacing = 20;
const lineHeight = crtFontSize + monitorLineSpacing;

export function screenRenderLoop(renderState: RenderState) {
    return () => {
        // update text first, then render
        ...
        const lastLine = lineHeight * (codeText.length);
        if (lastLine > textureCanvas.height * 0.9) {
            offset.y = textureCanvas.height * 0.9 - lastLine;
        }
        const skip = Math.max(Math.floor(-offset.y / lineHeight) - 1, 0);
        ...
        for (let i = skip; i < codeText.length; i++) {
            ...
            cY = lineHeight * (i + 1) + offset.y;
            if (i < codeText.length - 1)
                tCtx.fillStyle = "#6e7681";
            else {
                tCtx.fillStyle = "rgba(255, 255, 255, 0.1)";
                const pY = lineHeight * i + offset.y;
                tCtx.fillRect(0, pY + 13, textureCanvas.width, 4);
                tCtx.fillRect(0, cY + 13, textureCanvas.width, 4);
                tCtx.fillStyle = "#fff";
            }
